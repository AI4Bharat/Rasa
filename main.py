...
def get_arg_parser():
    parser = argparse.ArgumentParser(description='Training and evaluation script for acoustic / e2e TTS model ')

    # dataset parameters
    parser.add_argument('--dataset_name', default='indictts')
    parser.add_argument('--language', default='ta')
    parser.add_argument('--wavs_path', type=str) # dataset_name, language #CHANGE
    parser.add_argument('--alt_wavs_path', type=str, default=None)
    ...
    parser.add_argument('--split', default=None) # eg. 80, 90, 95, ...
    ...
    parser.add_argument('--text_cleaner', default='multilingual_cleaners', choices=['multilingual_cleaners', 'english_cleaners'])
    ...
    parser.add_argument('--durations_cache_path', default=None)
    ...
    parser.add_argument('--use_language_embedding', default=False, type=str2bool)
    parser.add_argument('--use_emotion_embedding', default=False, type=str2bool)
    ...
    parser.add_argument('--teacher_force_using_external_durations', default=False, type=str2bool) # for fastspeech, fastpitch
    parser.add_argument('--train_dp_by_melspec_rmse', default=False, type=str2bool) # for fastspeech, fastpitch
    parser.add_argument('--condition_aligner_on_speaker_embeddings', default=False, type=str2bool) # for fastspeech, fastpitch
    ...
    parser.add_argument('--lr_scheduler_after_epoch', default=True, type=bool) # scheduler.step() after epoch/step

    # training - logging parameters 
    parser.add_argument('--wandb_project_name', default='indic-tts', type=str)
    ...
    parser.add_argument('--save_n_checkpoints', default=2, type=int)
...
def formatter_indictts(root_path, meta_file, **kwargs):  # pylint: disable=unused-argument
    ...
    with open(txt_file, "r", encoding="utf-8") as ttf:
        for idx, line in enumerate(ttf):
            cols = line.split("|")
            wav_file = os.path.join(args.wavs_path, cols[0] + ".wav")
            if not os.path.exists(wav_file):
                if args.alt_wavs_path is not None:
                    wav_file = os.path.join(args.alt_wavs_path, cols[0] + ".wav")
                    if os.path.exists(wav_file):
                        print(f"Skipping file: {cols[0]}.wav as it doesn't exist in the specified path")    
                else:
                    print(f"Skipping file: {cols[0]}.wav as it doesn't exist in the specified path")
            text = cols[1].strip()
            ...
            language_name, emotion_name = None, None
            if len(cols) > 3:
                language_name = cols[3].strip()
            if len(cols) > 4:
                emotion_name = cols[4].strip()
            items.append({"text": text, "audio_file": wav_file, "speaker_name": speaker_name, "language_name": language_name, "emotion_name": emotion_name})
    return items
...
def get_test_sentences(language):
    if language in ['ta', 'PS']:
        ...
        test_sentences = [
            "ਅਨਾੜੀ ਡਾਇਰੈਕਟਰ ਉੱਤੇ ਲੋਕਾਂ ਨੂੰ ਉਂਜ ਵੀ ਬਹੁਤਾ ਭਰੋਸਾ ਨਹੀਂ ਹੁੰਦਾ.",
...
def main(args):
    if args.speaker == 'all' and args.split:
        meta_file_train = f"metadata_train_{args.split}.csv"
        meta_file_val = f"metadata_test_{args.split}.csv"
    elif args.speaker == 'all':
        meta_file_train = f"metadata_train.csv"
        meta_file_val = f"metadata_test.csv"
    else:
    ...
    if args.lr_scheduler == 'NoamLR':
        lr_scheduler_params = {
            ...
            "scheduler_after_epoch": args.lr_scheduler_after_epoch
        }
        ...
        config = FastPitchConfig(
            **base_tts_config,
            model_args = ForwardTTSArgs(
                ...
                teacher_force_using_external_durations=args.teacher_force_using_external_durations,
                train_dp_by_melspec_rmse=args.train_dp_by_melspec_rmse,
                condition_aligner_on_speaker_embeddings=args.condition_aligner_on_speaker_embeddings,
            ...
            use_emotion_embedding=args.use_emotion_embedding,
            use_ssim_loss = args.use_ssim_loss,
            compute_f0=True,
            f0_cache_path=os.path.join(args.output_path, "f0_cache"),
            durations_cache_path=args.durations_cache_path,
    ...
    if args.use_language_embedding:
        language_manager = CommonManager()
        language_manager.set_ids_from_data(train_samples + eval_samples, parse_key="language_name")
    else:
        language_manager = None
    
    # set emotion manager
    if args.use_emotion_embedding:
        emotion_manager = CommonManager()
        emotion_manager.set_ids_from_data(train_samples + eval_samples, parse_key="emotion_name")
    else:
        emotion_manager = None
    
    print('set to ', args.use_emotion_embedding, args.use_language_embedding)
    # load model
    if args.model == 'glowtts':
        ...
        model = ForwardTTS(config, ap, tokenizer, speaker_manager=speaker_manager, language_manager=language_manager, emotion_manager=emotion_manager)
    elif args.model == 'tacotron2':
    ...
    config.num_languages = language_manager.num_labels
    if hasattr(config.model_args, 'num_languages'):
        config.model_args.num_languages = language_manager.num_labels
    else:
        config.num_languages = 1
    
    config.num_emotions = emotion_manager.num_labels
    if hasattr(config.model_args, 'num_emotions'):
        config.model_args.num_emotions = emotion_manager.num_labels
    else:
        config.num_emotions = 1
