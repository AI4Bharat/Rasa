...
#             idx = 0
#             for sen in sens:
#                 if external_durations is not None:
#                     ext_drs.append(external_durations[:, idx:idx+len(sen)])
#                     idx = len(sen)
#                 else:
#                     ext_drs.append(None) # set external_durations to None to use FastPitch's duration predictor
...
#                     speaker_name = "female" # remove after evaluating the model
...
#         # handle multi-lingaul (deprecated traditional language manager in this repo)
#         # language_id = None
...
#         language_id = None
#         language_name = ''
#         if self.tts_languages_file or hasattr(self.tts_model.language_manager, "ids"):
#             # print('[✓] Multi-lingual')
#             if language_name and isinstance(language_name, str):
#                 # get language idx from the language name
#                 language_id = self.tts_model.language_manager.ids[language_name]
#             # elif not language_name:
#             #     raise ValueError(
#             #         " [!] Look like you use a multi-language model. "
#             #         "You need to define either a `language_name` or a `language_wav` to use a multi-language model."
#             #     )
#             else:
#                 language_embedding = None
#         else:
#             if language_name:
#                 raise ValueError(
#                     f" [!] Missing languages.json file path for selecting language {language_name}."
#                     "Define path for language.json if it is a multi-language model or remove defined language idx. "
#                 )
...
#                 # get emotion idx from the emotion name
#                 emotion_id = self.tts_model.emotion_manager.ids[emotion_name]
#             elif not emotion_name:
#                 raise ValueError(
#                     " [!] Look like you use a multi-emotion model. "
#                     "You need to define either a `emotion_name` or a `emotion_wav` to use a multi-emotion model."
#                 )
#             else:
#                 emotion_embedding = None
#         else:
#             if emotion_name:
#                 raise ValueError(
#                     f" [!] Missing emotions.json file path for selecting emotion {emotion_name}."
#                     "Define path for emotion.json if it is a multi-emotion model or remove defined emotion idx. "
#                 )
...
#             for sen, ext_dr in zip(sens, ext_drs):
#                 # synthesize voice
#                 print ("external_speaker_embd (synthesizer.py): ", external_speaker_embd)
#                 outputs = synthesis(
...
#                     external_durations=ext_dr,
#                     emotion_id=emotion_id,
#                     external_speaker_embd = external_speaker_embd
#                 )
        ...
        if self.tts_speakers_file or hasattr(self.tts_model.speaker_manager, "ids"):
            ...
            if speaker_name and isinstance(speaker_name, str):
                if self.tts_config.use_d_vector_file:
                    ...
                    # get speaker idx from the speaker name
                    # speaker_name = "indictts_female"
                    # speaker_name = "female" # remove after evaluating the model
                    speaker_id = self.tts_model.speaker_manager.ids.get(speaker_name, 0)
