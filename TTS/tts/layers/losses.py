...
class ForwardTTSLoss(nn.Module):
    ...
    def forward(
        ...
        train_aligner=True,
        use_speaker_encoder_as_loss=False,
        train_dp_by_melspec_rmse=False,
        gt_spk_emb=None,
        syn_spk_emb=None
    ):
        loss = 0
        ...
        if train_dp_by_melspec_rmse:
            self.dur_loss_alpha = 0  # Don't use duration targets to train DP with RMSE loss (rely on MelSpec RMSE to update DP parameters instead.)
            self.pitch_loss_alpha = 0
