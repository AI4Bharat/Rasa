...
def _parse_sample(item):
    ...
    return text, wav_file, speaker_name, language_name, emotion_name 
...
class TTSDataset(Dataset):
    def __init__(
        ...
        Args:
            ...
            durations_cache_path (str): Path to cache with stored durations. Defaults to None.
        ...
        self.durations_cache_path = durations_cache_path
        if durations_cache_path is not None:
            self.durations_dataset = DurationsDataset(
                self.samples, durations_cache_path, precompute_num_workers=precompute_num_workers
            )
    ...
    def samples(self, new_samples):
        ...
        if hasattr(self, "durations_dataset"):
            self.durations_dataset.samples = new_samples
    ...
    def get_durations(self, idx):
        out_dict = self.durations_dataset[idx]
        item = self.samples[idx]
        assert item["audio_file"] == out_dict["audio_file"]
        return out_dict
    ...
    def get_token_ids(self, idx, text, lang_id):
        if self.tokenizer.use_phonemes:
            token_ids = self.get_phonemes(idx, text)["token_ids"]
        else:
            if self.use_cls:
                token_ids = self.tokenizer.text_to_ids(text, lang_id)
            else:
    ...
    def load_data(self, idx):
        ...
        durations = None
        if self.durations_cache_path is not None:
            durations = self.get_durations(idx)["durations"]
    ...
    def collate_fn(self, batch):
        ...
        if isinstance(batch[0], collections.abc.Mapping):
           ...
           # get pre-computed d-vectors
            ...
            # get numerical language ids from language names
            if self.language_id_mapping:
                language_ids = [self.language_id_mapping[ln] for ln in batch["language_name"]]
            else:
                language_ids = None

            # get numerical emotion ids from emotion names
            if self.emotion_id_mapping:
           ...
           # compute features
            ...
            # format durations
            if self.durations_cache_path is not None:
                durations = prepare_data(batch["external_durations"])
                # assert token_ids.shape[1] == durations.shape[1], f"[!] {token_ids.shape} vs {durations.shape}"
                durations = torch.FloatTensor(durations).contiguous()
            else:
                durations = None
...
class PhonemeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ids = self.compute_or_load(item["audio_file"], item["text"], item["language_name"])
        ph_hat = self.tokenizer.ids_to_text(ids, item["language_name"]) if self.use_cls else self.tokenizer.ids_to_text(ids)
        return {"text": item["text"], "ph_hat": ph_hat, "token_ids": ids, "token_ids_len": len(ids)}

    def __len__(self):
        return len(self.samples)

    def compute_or_load(self, wav_file, text, lang_id):
        ...
        try:
            ...
            if self.use_cls:
                ids = self.tokenizer.text_to_ids(text, lang_id)
            else:
...
class DurationsDataset:
    """Durations Dataset for using external durations computed from other models, for example, Wav2Vec2 
    ...
    Args:
        ...
        cache_path (str):
            Path to cache of durations values. 
    ...
    def __init__(
        ...
        self.cache_path = cache_path
        if cache_path is not None and not os.path.exists(cache_path):
            raise OSError('Durations cache path does not exist: ' + cache_path)

    def __getitem__(self, idx):
        item = self.samples[idx]
        durations = self.compute_or_load(item["audio_file"])
        return {"audio_file": item["audio_file"], "durations": durations}
    ...
    @staticmethod
    def create_durations_file_path(wav_file, cache_path):
        file_name = os.path.splitext(os.path.basename(wav_file))[0]
        durations_file = os.path.join(cache_path, file_name + "_w2v2_durations.npy")
    ...
    def compute_or_load(self, wav_file):
        ...
        compute pitch and return a numpy array of pitch values
        """
        durations_file = self.create_durations_file_path(wav_file, self.cache_path)
        if not os.path.exists(durations_file):
            raise OSError('Duration file not found ' + durations_file)
        else:
    ...
    def collate_fn(self, batch):
        ...
        durations_lens = [len(item["durations"]) for item in batch]
        durations_lens_max = max(durations_lens)
        durations_torch = torch.LongTensor(len(durations), durations_lens_max).fill_(self.get_pad_id())
        for i, duration_len in enumerate(durations_lens):
            durations_torch[i, :durations_lens] = torch.LongTensor(durations[i])
        return {"audio_file": audio_file, "durations": durations_torch, "durations_lens": durations_lens}

    def print_logs(self, level: int = 0) -> None:
        indent = "\t" * level
        print("\n")
        print(f"{indent}> DurationsDataset ")
        print(f"{indent}| > Number of instances : {len(self.samples)}")
