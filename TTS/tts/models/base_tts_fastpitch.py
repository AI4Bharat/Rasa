...
from TTS.tts.utils.commons import CommonManager, get_label_balancer_weights, get_label_manager
...
class BaseTTS(BaseTrainerModel):
    ...
    def get_aux_input(self, **kwargs) -> Dict:
        ...
        return {"speaker_id": None, "style_wav": None, "d_vector": None, "language_id": None, "emotion_id": None}
    ...
    def get_aux_input_from_test_setences(self, sentence_info):
        ...
        speaker_id, d_vector, language_id, emotion_id = None, None, None, None
        ...
        # get emotion id
        if hasattr(self, "emotion_manager") and config.use_emotion_embedding and emotion_name is not None:
    ...
    def get_sampler(self, config: Coqpit, dataset: TTSDataset, num_gpus=1):
        ...
        if getattr(config, "use_language_weighted_sampler", False):
            ...
            weights = get_label_balancer_weights(data_items) * alpha
    ...
    def get_data_loader(
        ...
        if is_eval and not config.run_eval:
            ...
            if hasattr(self, "language_manager") and self.language_manager is not None:
                print('has language manager')
                if hasattr(config, "model_args"):
                    language_id_mapping = self.language_manager.ids if config.model_args.use_language_embedding else None
                else:
                    language_id_mapping = self.language_manager.ids if config.use_language_embedding else None
            else:
                language_id_mapping = None

            # setup multi-emotion attributes
            if hasattr(self, "emotion_manager") and self.emotion_manager is not None:
                print('has emotion manager')
                if hasattr(config, "model_args"):
                    emotion_id_mapping = self.emotion_manager.ids if config.model_args.use_emotion_embedding else None
                else:
                    emotion_id_mapping = self.emotion_manager.ids if config.use_emotion_embedding else None
            else:
            ...
            dataset = TTSDataset(
                ...
                durations_cache_path=config.get("durations_cache_path", None),
    ...
    def _get_test_aux_input(
        ...
        aux_inputs = {
            ...
            "emotion_id": None 
            if not self.config.use_emotion_embedding
            else random.sample(sorted(self.emotion_manager.ids.values()), 1),
        }
        return aux_inputs
    ...
    def test_run(self, assets: Dict) -> Tuple[Dict, Dict]:
        ...
        for idx, sen in enumerate(test_sentences):
            outputs_dict = synthesis(
                ...
                emotion_id=aux_inputs["emotion_id"],
    ...
    def on_init_start(self, trainer):
        ...
        if self.language_manager is not None:
            ...
            trainer.config.languages_file = output_path
            # some models don't have `model_args` set
            if hasattr(trainer.config, "model_args"):
                trainer.config.model_args.languages_file = output_path
            trainer.config.save_json(os.path.join(trainer.output_path, "config.json"))
            print(f" > `languages.pth` is saved to {output_path}.")
            print(" > `languages_file` is updated in the config.json.")

        if self.emotion_manager is not None:
            output_path = os.path.join(trainer.output_path, "emotions.pth")
            self.emotion_manager.save_ids_to_file(output_path)
            trainer.config.emotions_file = output_path
            # some models don't have `model_args` set
            if hasattr(trainer.config, "model_args"):
                trainer.config.model_args.emotions_file = output_path
            trainer.config.save_json(os.path.join(trainer.output_path, "config.json"))
            print(f" > `emotions.pth` is saved to {output_path}.")
            print(" > `emotions_file` is updated in the config.json.")
