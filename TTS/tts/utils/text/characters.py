...
class CLSTokens(BaseCharacters):
    def __init__(
        self,
        characters: str = None,
        punctuations: str = None,
        ...
        self.is_unique = is_unique
        self.is_sorted = is_sorted
        cls_table = pd.read_csv("/nlsasfs/home/ai4bharat/praveens/ttsteam/repos/text2speech-fp/TTS/tts/utils/text/common_labels.tsv", sep="\t", index_col=0, header=None)
        self.cls_lookup = {}
        for _, row in cls_table.iterrows():
            phonetic_representation = row[1]
            for _, cell_value in row.iloc[2:].items():
                self.cls_lookup[cell_value] = phonetic_representation
        self.tokens = []
        for c in characters:
            if c in self.cls_lookup:
                self.tokens.append(self.cls_lookup[c])
            elif c not in string.punctuation:
                self.tokens.append(c)
        # self.tokens = [self.cls_lookup[c] for c in characters]
        super().__init__(self.characters, punctuations, pad, eos, bos, blank, is_unique, is_sorted)
