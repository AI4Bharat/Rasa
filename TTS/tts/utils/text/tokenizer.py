...
class CLSTokenizer(TTSTokenizer):
    """🐸TTS tokenizer to convert input characters to token IDs and back.
    ...
    def __init__(
        ...
        self.use_eos_bos = use_eos_bos
        self.not_found_characters = []
        self.cls_table = pd.read_csv("common_labels.tsv", sep="\t", index_col=0, header=None) # for cls to lang decoding
        self.reverse_lookup = {'ml': 2, 'ta': 3, 'te': 4, 'kn': 5, 'brx': 6, 'hi': 6, 'mr': 6, 'raj': 6, 'as': 7, 'bn': 7, 'mni': 7, 'gu': 8, 'or': 9} # cls to language decoding
    ...
    def encode(self, text: str, language: str) -> List[int]:
        ...
        token_ids = []
        for char in text:
            tok = self.cls_table.loc[self.cls_table[int(language)] == char, 1]
            try:
                idx = self.characters.char_to_id(tok)
                token_ids.append(idx)
            except KeyError:
                # discard but store not found characters
                if char not in self.not_found_characters:
    ...
    def decode(self, token_ids: List[int], lang_id: str) -> str:
        """Decodes a sequence of IDs to a string of text."""
        text = ""
        assert lang_id.isdigit(), "Please provide proper language id"
        for token_id in token_ids:
            tok = self.characters.id_to_char(token_id)
            text += self.cls_table.loc[self.cls_table[1]==tok, int(lang_id)]
        return text

    def text_to_ids(self, text: str, language: str) -> List[int]:  # pylint: disable=unused-argument
        """Converts a string of text to a sequence of token IDs.
    ...
    def ids_to_text(self, id_sequence: List[int], lang_id: str) -> str:
        """Converts a sequence of token IDs to a string of text."""
        return self.decode(id_sequence, lang_id)
